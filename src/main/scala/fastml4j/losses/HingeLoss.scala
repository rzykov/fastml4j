...
class HingeLoss(lambdaL2: Double) extends Loss {
  ...
  def pureLoss(weights: INDArray, dataSet: DataSet): INDArray = {
    val out = ((dataSet.getFeatureMatrix dot weights.T) * dataSet.getLabels.T).neg() + 1
  ...
  def gradient(weights: INDArray, dataSet: DataSet): INDArray = {
    ...
    val main = (dataSet.getFeatureMatrix muliColumnVector (dataSet.getLabels.T * mask).T * (-1.0)).sum(0)
